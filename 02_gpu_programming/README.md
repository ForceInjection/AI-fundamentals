...
- [CUDA / Metal 裸机安装指南](01_environment/03_cuda_metal_install_guide.md) - 不依赖容器的原生 CUDA Toolkit 与 Metal 驱动安装流程，适用于 macOS + Ubuntu 双平台。
...
- **进阶专题**：NUMA API、设备属性查询、Stream 并发、Launch 开销、CUDA Graphs、Reduction、Tensor Core GEMM、GPU 内存管理、Shared Memory 与 Bank Conflict、Warp-level Programming、Multi-GPU 编程、异步拷贝与 Pipeline、性能调优方法论 等共 13 篇，详见 [CUDA 目录](02_cuda/README.md)。
