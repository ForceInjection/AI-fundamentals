...
**权衡**：TP 是唯一能突破单卡显存上限的策略，代价是**通信量极高**——每层 Attention 和 FFN 之后都需要 AllReduce。在 NVLink 互联的同一节点内这可以接受（~450 GB/s，延迟 ~5μs），但跨节点时延迟上升 5-10 倍，性能不可接受。因此 TP 的硬约束是：TP size ≤ 单节点 GPU 数，TP 组内所有 GPU 必须在同一 NVSwitch 域内。
