...
HiCache 是 SGLang 自带的分层 KV Cache 架构，将 GPU/CPU/分布式后端统一为 L1-L3 三级缓存。详细分析见 **[SGLang 推理引擎](sglang/README.md)** 章节。

### 1.6 KV Block Manager

- **[KV Block Manager 分析](kv_cache/02_systems/kvbm/KVBM_Analysis.md)** ([配套 PPT](kv_cache/02_systems/kvbm/NVIDIA_Dynamo_KVBM_Architecture.pptx) / [可编辑 PPT](kv_cache/02_systems/kvbm/NVIDIA_Dynamo_KVBM_可编辑.pptx))：NVIDIA Dynamo KVBM 架构与显存管理机制。
...
- **[NIXL 网络存储介绍](kv_cache/02_systems/nixl/nixl_introduction.md)**：高性能网络存储架构，为 LMCache 等 KV Cache 系统提供 RDMA 与跨节点数据传输能力。

### 1.10 高级主题

- **[分层流水线技术](kv_cache/01_concepts/advanced/layerwise_pipeline.md)**：Layer-wise Pipeline 的调度策略与显存优化。

---

## 2. 并行策略

70B 的模型塞不进一张 GPU——解法是把模型拆开、把数据拆开、把计算拆开。DP、TP、PP、EP、SP 五种策略切的维度完全不同。

- **[大模型推理并行策略](parallelism/parallelism_strategies.md)**（[交互可视化](parallelism/parallelism_visual.html)）——DP（数据并行）、TP（张量并行）、PP（流水线并行）、EP（专家并行）、SP（序列并行）：谁切权重、谁切 KV Cache、vLLM/SGLang 工程实践、混合部署案例。
...
- **[vLLM Router 架构解析](vllm/routing/vllm_router.md)**：高性能、轻量级请求转发系统。
- **[vLLM Semantic Router](vllm/routing/vllm_semantic_router_deep_dive.md)**：基于语义的智能路由策略。
...
- **[模型部署索引](deployment/README.md)**：DeepSeek-V3 H20 部署、Qwen2-VL 昇腾部署的方案导航与 SLO 方法论。
- **[DeepSeek-V3 MoE 模型 vLLM 部署](deployment/deepseek_v3_moe_vllm_h20_deployment.md)**：NVIDIA H20 硬件部署方案与 SLO 验证。
- **[Qwen2-VL-7B 华为昇腾部署](deployment/qwen2_vl_7b_huawei.md)**：国产硬件平台的部署调优实践。
...
- **[显存估算索引](memory_calc/README.md)**：LLM 推理显存估算的理论、脚本与配置导航。
- **[LLM 显存占用分析与计算](memory_calc/memory_analysis.md)** ([配套 PPT](memory_calc/llm_memory_analysis.pptx))：覆盖模型参数、KV Cache 与激活值的全量显存估算方法。
- 配套计算脚本：`memory_calc/calculate_qwen3_memory.py`、`memory_calc/calculate_deepseek_v4_memory.py`

### 5.2 模型优化

- **[模型优化索引](model_optimization/README.md)**：投机解码与模型压缩工具链概览。
- **[NVIDIA 模型优化器](model_optimization/nvidia_model_optimizer.md)**：工具链详解与优化实践。
- **[图解投机解码](model_optimization/illustrated-speculative-decoding.md)**：核心思想与系统实现。
...
- **[参考设计索引](reference_design/README.md)**：
  ...
  - **场景与运维**：[多模态](reference_design/10-多模态推理优化.md)、[边缘计算](reference_design/11-边缘推理优化.md)、[安全合规](reference_design/09-安全性与合规性.md)、[实施最佳实践](reference_design/07-实施建议与最佳实践.md)、[检查清单](reference_design/13-实施检查清单.md)、[场景解答](reference_design/12-场景问题解答.md)、[参考资料](reference_design/08-参考资料与延伸阅读.md)、[总结展望](reference_design/14-总结与展望.md)。
...
- **[成本分析索引](cost_analysis/README.md)**：模块入口与概述。
