# 模型量化：从手算到代码，理解 INT8/INT4

量化是将浮点权重/激活值映射到低比特整数的压缩技术。核心矛盾是范围 vs 精度——scale 越小精度越高但覆盖范围越小，一个 outlier 就能拉大 scale 导致大量正常值被"挤压"到几个量化级别。解法依次递进：对称量化（权重，零点固定为 0）→ 非对称量化（激活值，zero_point 补偿偏斜分布）→ per-channel（各通道独立 scale）→ group-wise（INT4 必须，每 128 个值一组独立量化）→ GPTQ（逐列量化 + 逆 Hessian 误差补偿）。7B 模型 INT4 后仅需 ~3.5 GB，但当前 CANN 8.0.1 栈不支持 HF 模型本地量化推理。
...
本章聚焦理论学习，用 Python/NumPy 演示量化原理。
...
- 实际有 255 个可表示的非零值（加上 0 共 256 个）——因为负数比正数多一个量化级别（-128），这个不对称是为了让 256 = 2^8，恰好填满 8 位整数空间
...
LLM.int8() 论文发现：transformer 模型中约 0.1% 的特征维度包含了极大的 outlier 值（比其他维度大 10-100 倍），这些 outlier 如果和其他值一起量化，会严重破坏精度。解决方案：对 outlier 维度单独用 FP16，其余用 INT8。
...
| --------------- | -------------------------- | ---------- | -------------------- | --------------------- |
| **Per-Tensor**  | 整个矩阵共享 1 个 scale    | 1          | 4 bytes              | 受 outlier 通道影响大 |
| **Per-Channel** | 每行（输出通道）独立 scale | 4096       | 16 KB                | 各通道自主最优        |
...
通道 1: max(|w₁|) = 2.50
        ...
        （与 per-tensor 对通道 1 的结果相同）

通道 0 的 RMSE (per-channel): 0.0003  ← 比 per-tensor (0.0054) 提升 18×
通道 1 的 RMSE (per-channel): 0.0038  ← 与 per-tensor 相同（本来就是它主导的）

额外开销: 2 个 scale × 4 bytes = 8 bytes (可忽略)
```

**关键洞察**：per-channel 让每个通道按自己的范围选择 scale——通道 0 的 scale 缩小了 25 倍（0.000787 vs 0.01969），精度同等提升。而存储开销（每行 4 字节的 scale）对于 4096×4096 的权重矩阵来说只是 16 KB，完全可以忽略。

LLM 推理中，**权重用 per-channel 对称量化**，**激活值用 per-tensor 非对称量化**是标准配置。
...
#### 6.1.1 手算示例：INT4 Per-Tensor vs Group-Wise
...
**Per-Tensor INT4**（全部 8 个值共享一个 scale）：
...
Group 1 (后4): max(|w|) = 2.10, scale₁ = 2.10 / 7 = 0.30
  (与 per-tensor 对这组的结果相同)
  RMSE: 0.039

额外开销: 2 个 scale × 4 bytes = 8 bytes（总共 8 个值 × 4bit = 4 bytes 的数据）
```

**结论**：group-wise INT4 用极小的存储代价（每 128 个值 1 个 scale，增加约 3% 存储），换回了 per-channel 级别的精度。这就是为什么 INT4 量化必须搭配 group-wise。

### 6.2 GPTQ 的核心思想

GPTQ（Generalized Post-Training Quantization）是一种数据相关的权重量化算法。它不只是一个 CUDA 实现——首先是一个算法思想。

**问题**：简单地对整个权重矩阵做 per-channel 量化，虽然每个 channel 各自最优，但不同 channel 的量化误差会累积，最终对模型输出产生不可忽略的影响。
...
**为什么需要 Hessian 矩阵**：H^{-1} 编码了模型对每个权重的敏感度——敏感度高的权重，量化误差影响大，需要更精细的补偿。如果没有 Hessian 指导（即原始 OBQ 算法），补偿是盲目的；GPTQ 用二阶信息确保误差补偿到正确的位置。

**GPTQ 的一个 epoch**：对整个权重矩阵扫描一遍即可——不需要多轮迭代。这使得量化一个 7B 模型可以在数分钟内完成（需要一张 GPU/NPU 跑校准数据 + CPU 做量化计算）。

GPTQ 本身是算法——可以用 NumPy 实现。bitsandbytes 和 AWQ 是 GPTQ 思路的不同工程实现（CUDA kernel + 不同的量化粒度选择）。
...
但加速需要**硬件原生支持** INT8/INT4 计算。当前 CANN 8.0.1 的 Cube 单元支持 INT8，但 torch_npu 2.1.0 没有暴露 HuggingFace 模型的 INT8 推理路径。
