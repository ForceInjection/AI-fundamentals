...
量化是将浮点权重/激活值映射到低比特整数的压缩技术。核心矛盾是范围 vs 精度——scale 越小精度越高但覆盖范围越小，一个 outlier 就能拉大 scale 导致大量正常值被"挤压"到几个量化级别。解法依次递进：对称量化（权重，零点固定为 0）→ 非对称量化（激活值，zero_point 补偿偏斜分布）→ 逐通道量化（各通道独立 scale）→ group-wise（INT4 必须，每 128 个值一组独立量化）→ GPTQ（逐列量化 + 逆 Hessian 误差补偿）。7B 模型 INT4 后仅需 ~3.5 GB，但当前 CANN 8.0.1 栈不支持 HF 模型本地量化推理。
